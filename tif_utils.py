...
class TifProcessor:
    ...
    def trim_to_western_us(self, ds, lon_boundary=-100.0, quadrant=None):
        """
...
# these are old functions that are no longer needed
"""
    def generate_land_mask(self, lon, lat):
        # Load Natural Earth land geometries
        shp_path = natural_earth(resolution='110m', category='physical', name='land')
        ...
        # Boolean mask: True where point is on land
        return shapely.vectorized.contains(land_geom, Lon, Lat)
    
    
    
    def generate_conus_mask(self, lon, lat):
       # Load country boundaries and isolate the U.S.
       shp_path = natural_earth(resolution='110m', category='cultural', name='admin_0_countries')
       countries = gpd.read_file(shp_path)
       usa = countries[countries['ADMIN'] == 'United States of America']
    
       # Define CONUS bounding box (excludes Alaska, Hawaii, territories)
       conus_bounds = box(-125, 24, -66.5, 50)
       conus_gdf = gpd.GeoDataFrame(geometry=[conus_bounds], crs=usa.crs)
    
       # Intersect U.S. with CONUS box
       usa_conus = gpd.overlay(usa, conus_gdf, how='intersection')
    
       # Get unioned geometry for masking
       conus_geom = usa_conus.unary_union
    
       # Create coordinate grid
       Lon, Lat = np.meshgrid(lon, lat)
    
       # Return boolean mask: True where inside CONUS
       return shapely.vectorized.contains(conus_geom, Lon, Lat)   


    
    def plot_ds_epsg4326(self,
        ds, var_name, colormap,
        bounding_box=None,  # (min_lon, max_lon, min_lat, max_lat)
        save_path=None, grid_thickness="0", show_colorbar=True,
        ...
        lon = ds['lon'].values
        lat = ds['lat'].values
        z = ds[var_name].values.copy()
    
        # Mask ocean: keep land, mask everything else
        #land_mask = self.generate_land_mask(lon, lat)
        land_mask = self.generate_conus_mask(lon, lat)
    ...
    def old_mask_non_conus_and_save(self, ds, var_name, output_nc):
    ...
    def mask_non_conus_and_save(self, ds, var_name, output_nc):
        # Load USA country boundaries
        country_path = natural_earth(resolution='110m', category='cultural', name='admin_0_countries')
        ...
        # Load major lakes shapefile
        lakes_path = natural_earth(resolution='110m', category='physical', name='lakes')
        lakes = gpd.read_file(lakes_path)
        lakes = lakes.to_crs(usa.crs)
    
        # Restrict lakes to CONUS bounding box
        lakes_conus = lakes[lakes.intersects(conus_geom)]
        lakes_geom = lakes_conus.unary_union
        ...
        # Mask for CONUS
        conus_mask = shapely.vectorized.contains(conus_geom, Lon, Lat)
    
        # Mask for Lakes
        lake_mask = shapely.vectorized.contains(lakes_geom, Lon, Lat)
    
        # Final mask: CONUS but not in lakes
        final_mask = conus_mask & (~lake_mask)
    
        # Apply mask
        data = ds[var_name].values
        data_masked = np.where(final_mask, data, np.nan)
    ...
    def old_plot_ds_epsg4326(
        ...
        # Define value bins and corresponding colors
        boundaries = [0, 64, 81, 97, 113, 161, 178, 193, 253, 254, 255, 256]
        colors = [
            '#800000',  # 0-63 Maroon
            '#FF0000',  # 64-80 Red
            '#FFA500',  # 81-96 Orange
            '#FFFF00',  # 97-112 Yellow
            '#FFFFFF',  # 113-160 White
            '#90EE90',  # 161-177 Light Green
            '#008000',  # 178-192 Green
            '#006400',  # 193-252 Dark Green
            '#00008B',  # 253 Dark Blue
            '#808080',  # 254 Gray
            '#FFFFFF',  # 255 White
        ]
        ...
        # Show colorbar if specified
        if show_colorbar:
            cbar = plt.colorbar(img, ax=ax, orientation='vertical', pad=0.05, boundaries=boundaries)
            cbar.set_label(var_name)
            cbar.set_ticks(boundaries[:-3])  # Exclude 253-255 from colorbar ticks
